...
from utils.auth_utils import get_current_user_id_from_jwt, is_enterprise_user
...
async def get_allowed_models_for_user(client, user_id: str):
    ...
    """
    if is_enterprise_user(user_id):
        # Enterprise users get all models
...
async def can_use_model(client, user_id: str, model_name: str):
    ...
    if is_enterprise_user(user_id):
        # Enterprise users can use any model
        return True, "Enterprise user: all models allowed", list(set(MODEL_NAME_ALIASES.values()))
...
async def check_billing_status(client, user_id: str) -> Tuple[bool, str, Optional[Dict]]:
    ...
    if is_enterprise_user(user_id):
        # Enterprise users have infinite usage
        return True, "Enterprise user: unlimited usage", {
            "price_id": "enterprise",
...
async def get_available_models(
    ...
    try:
        ...
        if is_enterprise_user(current_user_id):
            # Enterprise users get all models, all available
            model_info = []
            for short_name, full_name in MODEL_NAME_ALIASES.items():
                model_info.append({
                    "id": full_name,
